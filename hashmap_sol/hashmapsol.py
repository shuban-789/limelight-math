...
def get_diag_pair(corners):
    ...
    for corner in corners[1:]:
        distances.append(distance(reference[0], reference[1], corner[0], corner[1]))
        hashmap[distance(reference[0], reference[1], corner[0], corner[1])] = [corner, reference]
    return hashmap[max(distances)]

# 4 corners should mean distances is of length 3. <diagonal, long, short>
def get_long_pair():
    long_array = distances.copy()
    long_array.sort(reverse=True)
    long_dist = long_array[1]
    return hashmap[long_dist]
